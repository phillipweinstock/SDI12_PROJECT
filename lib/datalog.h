...
/* Create an rtc object and select Slowclock source
    ...
    This is not idea and is just for demo purposes, I cannot get the external rtc to work on the due, gd knows why...
 */
// RTCDue rtc(RC);
RTCDue rtc(XTAL);
enum SD_settings
...
void logsensordata()
{
    ...
    sprintf(logbuffer, "%u,%2.2f,%2.2f,%u,%u,%u", rtc.unixtime(), output.temp, output.humidity, output.pressure, lightoutput.lux, output.gasresistance);
